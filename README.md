...
1. 사이트를 열고 로그인 화면 아래 **비밀번호 해시 만들기** 를 누릅니다.
2. 쓰고 싶은 비밀번호를 입력하면 64자리 값이 나옵니다. **복사** 를 누릅니다.
3. 그 값을 `ADMIN_PW_SHA256` 에 붙여넣고 커밋합니다.

```js
ADMIN_PW_SHA256 : '3f9a…64자리…c1',
```

이후 아이디 `ohh5259` 로 그 비밀번호를 넣어야만 로그인됩니다.
비밀번호 원문은 어디에도 저장되지 않습니다.

> 아이디에서 유추되는 비밀번호(`ohh5259**` 같은)는 피하세요. 이 저장소는 공개라 코드가 누구에게나 보입니다.
...
Worker 모드에서는 `ADMIN_PW_SHA256` 을 쓰지 않습니다. 대신 **`ADMIN_ID` 로 지정한 아이디가 가입하면 자동으로 관리자**가 됩니다.
